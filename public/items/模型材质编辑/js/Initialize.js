...
var Editor = function () {
	...
	function init3DMesh(opts) {
		var helper = new THREE.GridHelper(2000, 80, 0x43908d, 0x3a3b3b);
	...
	thm.uploadModel = (url) => {
		...
		loadFbxModel(url, (obj) => {
			...
			obj.traverse((elem) => {
				const hasMatArr = [];
				const hasMatNames = [];
				/* 处理r118版加载模型有多材质时报错问题 */
				/* obj.traverse((child) => {
					if (child.geometry && child.material) {
						if (Array.isArray(child.material)) {
							child.material.forEach(mat => {
								if (!hasMatNames.includes(mat.name)) {
									hasMatArr.push(mat.clone());
									hasMatNames.push(mat.name);
									mat.dispose();
									if (mat.map) {
										mat.map.dispose();
									}
							...
							child.material.forEach(mat => {
								const index = hasMatNames.indexOf(mat.name);
								if (index != -1) {
									mats.push(hasMatArr[index]);
								}
							});
	...
	function onDocumentMouseMove(event) {
		/* var intersects = rayEvnet(event, thm.rayMesh.concat([]));
	...
	function onDocumentMouseDown(event) {
		...
		if (intersects.length != 0 && event.buttons == 1) {
			...
			VM.updateFolder(object.material, object.name);
		} else {
		}
	}
